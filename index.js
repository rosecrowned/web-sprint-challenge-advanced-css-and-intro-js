...
let artists[8].name = "Vincent Van Gogh";
...
function get20s(artists,year){
 let lastCentury= [];
  for (i =0; i < artists.length; i++){
    let year = artists[i].split(" - "){
      if ((artists.years[0])<=1900) || artists.years[1] <=2000)){
        lastCentury.push(artists[i].name);
      }
    }
  }
  return lastCentury;
...
function addArtist(array,id,name, years, genre, nationality, bio){
   artists.[push(artists, 'Shannon', 1988 - 2021, 'Web Design', 'White', "Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit.")]
   return artists;
  }
...
function lotsOfArt(artists){
  let totalArt = [];
  let (i = 0; i < artists.length; i++){
    if (artists[i[.paintings > 100){
      totalArt.push(artists[i].name);
    }
  }
  return totalArt;
}
